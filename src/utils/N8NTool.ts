export default class N8NTool {
    ...
    static async readFile(name: string, prePath: string = "") {
        try {
            const res = await fetch(`http://localhost:5678/webhook/getCacheByName?name=${name}&path=${N8NTool.CachePath + prePath}`);
    ...
    static async writeFile(name: string, content: any, prePath: string = "") {
        try {
            ...
            const res = await fetch(`http://localhost:5678/webhook/setCacheByName`, {
                method: "POST",
                body: formData,
            });
            return await res.json();
